...
const app = new Hono()
        .get(
                ...
                async (c) => {
                        ...
                        const assisgneeIds = tasks.documents.map((task) => task.assigneeId);
                        ...
                        const members = await databases.listDocuments(
                                DATABASE_ID,
                                MEMBERS_ID,
                                assisgneeIds.length > 0 ? [Query.contains('$id', assisgneeIds)] : [],
                        );
        ...
        .post(
                ...
                async (c) => {
                        ...
                        if (workspaceIds.size !== 1) {
                                return c.json({ message: 'All task must be in the same workspace' }, 400);
                        }
